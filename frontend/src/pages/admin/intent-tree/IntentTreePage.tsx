...
function IntentNodeDialog({
  ...
  const parentOptions = useMemo(() => {
    const filtered =
      mode === "edit" && node
        ? treeOptions.filter((item) => item.value !== node.intentCode)
        : treeOptions;
    ...
    try {
      if (mode === "create") {
        const payload: IntentNodeCreatePayload = {
          ...
          paramPromptTemplate:
            values.kind === 2 ? values.paramPromptTemplate?.trim() || undefined : undefined
        };
        ...
        const payload: IntentNodeUpdatePayload = {
          ...
          examples: examples.length > 0 ? examples : undefined,
          collectionName:
            values.kind === 0 ? values.collectionName?.trim() || undefined : undefined,
          mcpToolId: values.kind === 2 ? values.mcpToolId?.trim() || undefined : undefined,
          kind: values.kind,
          topK: values.topK ?? undefined,
          sortOrder: values.sortOrder ?? 0,
