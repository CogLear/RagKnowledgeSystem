...
const TrafficOverviewSection = ({
  ...
  const chartSeries: TrendSeries[] = useMemo(() => {
    if (!trends.messages?.series?.length) return [];
    return trends.messages.series.map((s) => ({
      name: s.name,
      data: s.data.map((p) => ({ ts: p.ts, value: p.value }))
    }));
  }, [trends.messages]);
  ...
  return (
    <div className="rounded-xl bg-slate-50 p-4">
      ...
      <div className="h-48 relative">
        <InteractiveChart title={title} timeWindow={timeWindow} series={series} loading={loading}>
          <SimpleLineChart
            series={series}
            xAxisMode={xAxisMode}
            yAxisType={yAxisType}
