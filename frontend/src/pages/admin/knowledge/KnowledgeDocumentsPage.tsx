...
export function KnowledgeDocumentsPage() {
  ...
  const detailOverlapChars =
    detailTarget?.overlapChars ??
    getConfigNumber(detailConfig, "overlapChars", DEFAULT_OVERLAP_CHARS);
  const detailNameChanged = detailTarget
    ? detailName.trim() !== (detailTarget.docName || "")
    : false;
    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-subtitle">{kb ? `${kb.name}（${kb.collectionName}）` : kbId}</p>
        </div>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => (!open ? setChunkTarget(null) : null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {chunkTarget?.chunkCount ? "重新分块？" : "开始分块？"}
            </AlertDialogTitle>
            <AlertDialogDescription>
      ...
      <Dialog
        ...
        onOpenChange={(open) => (!open ? setDetailTarget(null) : null)}
      >
      ...
      <Dialog
        ...
        onOpenChange={(open) => (!open ? setLogTarget(null) : null)}
      >
        <DialogContent className="max-h-[90vh] overflow-y-auto sidebar-scroll sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>分块详情</DialogTitle>
            <DialogDescription>文档 [{logTarget?.docName}] 的分块执行日志</DialogDescription>
          </DialogHeader>
          {logLoading ? (
            <div className="py-8 text-center text-muted-foreground">加载中...</div>
                      ...
                      {!isPipelineLog && (
                        <div>
                          ...
                          <span className="font-medium">
                            {formatDuration(log.embeddingDuration)}
                          </span>
                        </div>
                      )}
                      <div>
  ...
  const handleSubmit = async (values: UploadFormValues) => {
    ...
    try {
      const payload: KnowledgeDocumentUploadPayload = {
        ...
        chunkSize:
          values.processMode === "chunk" && values.chunkStrategy === "fixed_size"
            ? chunkSize
            : null,
        overlapSize:
          values.processMode === "chunk" && values.chunkStrategy === "fixed_size"
            ...
            : null,
        targetChars:
          values.processMode === "chunk" && values.chunkStrategy === "structure_aware"
            ? targetChars
            : null,
        maxChars:
          values.processMode === "chunk" && values.chunkStrategy === "structure_aware"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>分块策略：直接分块；数据通道：使用Pipeline清洗</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
                  ...
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue
                            placeholder={loadingPipelines ? "加载中..." : "选择数据通道"}
                          />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                          ...
                          <FormControl>
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                className={
                                  noChunk ? "bg-slate-100 border-slate-400 font-medium" : ""
                                }
                              >
